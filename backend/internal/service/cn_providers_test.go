...
func TestNormalizeDeepSeekResponsesRequestBody(t *testing.T) {
	...
	deepseekResponses.Credentials["api_protocol"] = APIProtocolAdaptive
	adaptiveBody := normalizeDeepSeekResponsesRequestBody(deepseekResponses, body)
	require.False(t, gjson.GetBytes(adaptiveBody, "store").Bool())
	require.False(t, gjson.GetBytes(adaptiveBody, "previous_response_id").Exists())
