...
// UpstreamBalanceBaseURL returns the canonical site address used by the CC Switch
// usage endpoint. It is safe to expose to administrators as a link, unlike the
// account credentials used to make the upstream request.
...
func upstreamBalanceQueryForAccount(account *Account) *UpstreamBalanceQuery {
	if query := decodeUpstreamBalanceQuery(account.Extra); query != nil {
		return query
	}
	// CC Switch exposes this endpoint for its compatible API-key sites. The
	// overview uses it by default, while an account can still opt into a custom
	// query when the upstream has a different contract.
	return &UpstreamBalanceQuery{Preset: UpstreamBalancePresetCCSwitch}
}
