...
func TestCNProviderQuotaService_ZhipuTeamHeaders(t *testing.T) {
	for _, organization := range []string{"", "test-org"} {
		repo := &fakeCNProbeAccountRepo{account: &Account{
			ID: 4, Platform: PlatformZhipu, Type: AccountTypeAPIKey,
			Credentials: map[string]any{"account_mode": "coding", "api_key": "test-key", "zhipu_organization": organization, "zhipu_project": "test-project"},
		}}
		upstream := &recordingHTTPUpstream{}
		svc := NewCNProviderQuotaService(repo, nil, upstream, &config.Config{})
		_, _ = svc.QueryUsage(context.Background(), 4)
		require.NotNil(t, upstream.request)
		require.Equal(t, organization, upstream.request.Header.Get("bigmodel-organization"))
		if organization != "" {
			require.Equal(t, "2", upstream.request.URL.Query().Get("type"))
			require.Equal(t, "test-project", upstream.request.Header.Get("bigmodel-project"))
		} else {
			require.Empty(t, upstream.request.URL.RawQuery)
			require.Empty(t, upstream.request.Header.Get("bigmodel-project"))
		}
	}
}
