...
func allowOpenAICompatibleMessagesDispatch(apiKey *service.APIKey, contexts ...*gin.Context) bool {
	var c *gin.Context
	if len(contexts) > 0 {
		c = contexts[0]
	}
	...
	for {
		...
		if err != nil {
			if result != nil && result.ImageCount > 0 {
				...
				// 取连接期间客户端取消不代表账号故障，也不能再写兜底 502。
				if errors.Is(err, context.Canceled) && failoverClientGone(c) {
					reqLog.Info("openai.forward_aborted_client_disconnected", zap.Int64("account_id", account.ID))
					return
				}
...
func (h *OpenAIGatewayHandler) Messages(c *gin.Context) {
	...
	if !allowOpenAICompatibleMessagesDispatch(apiKey, c) {
...
func (h *OpenAIGatewayHandler) handleAnthropicFailoverExhausted(c *gin.Context, failoverErr *service.UpstreamFailoverError, streamStarted bool) {
	...
	if failoverErr != nil && service.IsUpstreamBillingError(failoverErr.StatusCode, failoverErr.ResponseBody) {
		service.SetOpsUpstreamError(c, failoverErr.StatusCode, service.ExtractUpstreamErrorMessage(failoverErr.ResponseBody), "")
		h.anthropicStreamingAwareError(c, http.StatusServiceUnavailable, "api_error", service.UpstreamGroupUnavailableClientMessage, streamStarted)
		return
	}
...
// wrapOpenAIAccountRelease 将账号槽位保留到 Forward 返回，并保证只释放一次。
// 客户端断开后上游仍可能继续执行，不能提前把这部分容量提供给新请求。
func wrapOpenAIAccountRelease(release func()) func() {
	if release == nil {
		return nil
	}
...
func (h *OpenAIGatewayHandler) acquireResponsesAccountSlot(
	...
	if selection.Acquired {
		return wrapOpenAIAccountRelease(selection.ReleaseFunc), true
	}
	...
	if fastAcquired {
		...
		return wrapOpenAIAccountRelease(fastReleaseFunc), true
	}
	...
	return wrapOpenAIAccountRelease(accountReleaseFunc), true
}
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	if apiKey.Group != nil && apiKey.Group.Platform == service.PlatformComposite {
		...
		if !ok || !isOpenAICompatibleTextPlatform(platform) {
...
func (h *OpenAIGatewayHandler) handleFailoverExhausted(c *gin.Context, failoverErr *service.UpstreamFailoverError, streamStarted bool) {
	...
	if service.IsUpstreamBillingError(statusCode, responseBody) {
		service.SetOpsUpstreamError(c, statusCode, service.ExtractUpstreamErrorMessage(responseBody), "")
		h.handleStreamingAwareError(c, http.StatusServiceUnavailable, "upstream_error", service.UpstreamGroupUnavailableClientMessage, streamStarted)
		return
	}
