...
const templateOptions = computed(() => {
  const items = templatesCache.value.filter((t) => {
    ...
    if (!supportsMonitorAPIMode(form.provider)) return true
...
function templateOptionLabel(tpl: ChannelMonitorTemplate): string {
  if (!supportsMonitorAPIMode(tpl.provider)) return tpl.name
...
watch(() => form.provider, () => {
  ...
  if (!supportsMonitorAPIMode(form.provider)) {
...
async function openMyKeyPicker(index = 0) {
  keyPickerIndex = index
...
function pickMyKey(k: ApiKey) {
  form.probes[keyPickerIndex].api_key = k.key
