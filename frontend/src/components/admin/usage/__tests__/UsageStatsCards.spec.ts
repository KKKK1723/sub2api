...
describe('UsageStatsCards', () => {
  it('shows cache token breakdown values', () => {
    ...
    expect(text).toContain('Cache hit rate: 16.42%')
  })

  it('shows a zero cache hit rate when there are no prompt tokens', () => {
    const wrapper = mount(UsageStatsCards, {
      props: {
        stats: {
          ...stats,
    ...
    expect(wrapper.text()).toContain('Cache hit rate: 0.00%')
  })
})
